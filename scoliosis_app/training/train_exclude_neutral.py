"""
Training script for ScoNet models on Scoliosis1K dataset (excluding the neutral class)
...
class Trainer:
    ...
    def validate(self, epoch, loader, phase='Val'):
        ...
        self.writer.add_scalar(f'{phase}/Sensitivity_Negative', recall[1], epoch)
        ...
        if phase == 'Val':
            ...
            print(f"Sensitivity - Positive: {recall[0]:.4f}, Negative: {recall[1]:.4f}")
    ...
    def save_checkpoint(self, epoch, val_acc, val_sensitivity):
        ...
        if val_sensitivity > self.best_val_sensitivity and val_acc > 0.75:
        ...
        #     best_path = self.checkpoint_dir / 'best_accuracy.pth'
        #     torch.save(checkpoint, best_path)
...
def test(self, epoch):
    ...
    # # (unchanged) Load best checkpoint
    # checkpoint = torch.load(self.checkpoint_dir / 'best.pth', weights_only=False)
    ...
    # Save test results (extended with AUC)
    results = {
        'test_auc': float(test_auc),
        'test_accuracy': float(test_acc),
        'test_sensitivity': float(recall[0]),
    ...
    print(f"Sensitivity (Positive): {recall[0]:.4f}")
...
def get_config():
    ...
    config = {
        ...
        # Training
        'epochs': 100,  # EPOCH NUM - change this later
        'learning_rate': 1e-4,
        'weight_decay': 1e-5,
        'patience': 10,
